package com.decent.bigdata.qz.service

import com.alibaba.fastjson.JSONObject
import com.decent.bigdata.qz.bean.{DwdQzPaperView, DwdQzPoint, DwdQzQuestion}
...
object EtlDataService {
  ...
  def etlQzChapter(ssc: SparkContext, sparkSession: SparkSession) = {
    ...
    ssc.textFile("/user/atguigu/ods/QzChapter.log").filter(item => {
      val obj = JsonDataParse.getData(item)
      obj.isInstanceOf[JSONObject]
    }).mapPartitions(partitions => {
      partitions.map(item => {
        val jsonObject = JsonDataParse.getData(item)
