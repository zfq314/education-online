...
import com.decent.bigdata.warehouse.bean.QueryResult
import com.decent.bigdata.warehouse.dao.DwsMemberDao
...
object AdsMemberService {
	...
	def queryDetailApi(sparkSession: SparkSession, dt: String) = {
		...
		val result = DwsMemberDao.queryIdlMemberData(sparkSession)
			.as[QueryResult].where(s"dt='${dt}'")
		result.cache()
		//统计注册来源url人数
		val a = result.mapPartitions(partition => {
			partition.map(data => (data.appregurl + "_" + data.dn + "_" + data.dt, 1))
		}).groupByKey(_._1)
			.mapValues(data => data._2).reduceGroups(_ + _)
			.map(data => {
				val keys = data._1.split("_")
				val appregurl = keys(0)
				val dn = keys(1)
				val dt = keys(2)
				(appregurl, data._2, dt, dn)
			}).toDF().coalesce(1).write.mode(SaveMode.Overwrite).insertInto("ads.ads_register_appregurlnum")
		//统计所属网站人数
		result.mapPartitions(partition => {
			partition.map(data => (data.sitename + "_" + data.dn + "_" + data.dt, 1))
		}).groupByKey(_._1).mapValues((data => data._2)).reduceGroups(_ + _)
			.map(data => {
				val keys = data._1.split("_")
				val sitename = keys(0)
				val dn = keys(1)
				val dt = keys(2)
				(sitename, data._2, dt, dn)
