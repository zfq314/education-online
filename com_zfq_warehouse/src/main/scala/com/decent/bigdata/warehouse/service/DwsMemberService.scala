...
import com.decent.bigdata.warehouse.bean.{DwsMember, DwsMember_Result, MemberZipper, MemberZipperResult}
import com.decent.bigdata.warehouse.dao.DwdMemberDao
...
object DwsMemberService {
	...
	def importMemberUseApi(session: SparkSession, dt: String): Unit = {
		...
		//主用户表
		//基础表的查询
		val dwdMember = DwdMemberDao.getDwdMember(session).where(s"dt='${dt}'")
		...
		val resultData = result.groupByKey(data => data.uid + "_" + data.dn)
			.mapGroups {
				case (key, iters) =>
					...
					val ad_id = dwsMembers.map(_.ad_id).head //iterator.next()
					...
					DwsMember_Result(
						uid, ad_id, fullname, icounurl, lastlogin, mailaddr, memberlevel, password, paymoney,
